...
// структура для приоритетности вершин
struct Priority {
	ll distance, vertex;
};
...
// перегрузка оператора для хранения в очереди
...
int main()
{
	...
	while (!pq.empty()) // пока очередь не пуста
	{
		Priority u = pq.top();
		pq.pop(); 
		if (relaxed[u.vertex]) // если не прорелаксированна
			continue;
		relaxed[u.vertex] = 1;

		for (Edge& e : edges[u.vertex]) // проход по вершинам
		{
			// если дистанцию можно уменьшить, то сохраняем
